...
impl TemplateManager {
    ...
    fn verify_or_create_checksum(&self, template_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !checksum_path.exists() {
            println!("{} Creating missing checksum for {}", 
                "→".blue(), 
                template_path.display().to_string().yellow());
    ...
    fn update(&self, lang: &str, agent: &str, force: bool) -> Result<(), Box<dyn std::error::Error>> {
        println!("{} Updating templates for {} with {}", 
            "→".blue(), 
            lang.green(), 
            agent.green());
    ...
    fn clear(&self, force: bool) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if self.config_dir.exists() {
            println!("{} Clearing templates from {}", 
                "→".blue(), 
                self.config_dir.display().to_string().yellow());
